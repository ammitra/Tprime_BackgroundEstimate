'''
Script to generate a 4x4 grid of plots for 1D limits as a function of mPhi for different mT values.
...
def plot_limit_1D(ax, x_axis_vals, xMass, m2, m1, med, p1, p2, obs):
    ...
    ax.set_yscale('log')
    #ax.set_xscale('log')
    ax.set_xlim([75, 500])    # the x-axis is plotting mY 
    ...
    ax.text(0.3, 0.95, r'$m_{T^\prime} = %s$ GeV'%(xMass), ha='center', va='top', fontsize='small', transform=ax.transAxes)
